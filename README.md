...
[![Firmware](https://img.shields.io/badge/Firmware-V15-brightgreen)]()
[![Cloud](https://img.shields.io/badge/Cloud-OneNET%20Studio-00B4D8)]()
[![License](https://img.shields.io/badge/License-MIT-lightgrey)]()

> **V15** (2026-06-16) — STM32F103C8T6 + ESP8266-01 的 LCC-S 谐振全桥无线供电系统。**Dual-MCU 双脑架构** + **系统全局状态机** + **圆弧能量条仪表盘** + OneNET MQTT 多端控制。应用场景：植入式医疗设备无线充电。
...
- **全局系统状态机 (V14)**: INIT→IDLE→SWEEP→RUNNING→FAULT, 按状态子集调度 Task
- **TFT 彩屏 9 页面**: 两级菜单 + 仪表盘 + 配网 + 故障, 200ms 增量刷新
- **双 EMA 滤波链**: Sys_Safety 安全级滤波 (V/I) + Ui_Controller 显示级滤波, 频率无 EMA 零迟滞
- **安全剥离 (V14)**: PB10 电源控制 + 过流检测从 UI 解耦到 Sys_Safety 独立模块
...
| 版本 | 日期 | 主要变更 |
|:---|:---|:---|
| V9 | 2026-06-11 | TFT 彩屏 9 页面 + 代码审查 37 项修复 |
| V10 | 2026-06 | 两级菜单架构 + 频率步进修复 |
| V11 | 2026-06 | 环形仪表盘 (sin查表极坐标 + Bres_Line指针 + 金属Hub) |
| V12 | 2026-06 | 移除连线弧 + 指针锥形化 + 动态锚点对齐 |
| V13 | 2026-06 | GaugeConfig 整十对齐 + Draw_TopRight_Icons 解耦 |
| **V14** | 2026-06-16 | **全局系统状态机 + Sys_Init/Sys_Safety/Sys_Run 模块化 + 静态函数全命名规范** |
| **V15** | **2026-06-16** | **圆弧能量条仪表盘 + 信息舱 + 电流 0-2A + CLAUDE.md 全面重写** |
...
| 分支 | 版本 | 显示 | 架构 |
|:---|:---:|:---:|:---|
| `4.0TFT` | V15 | TFT 彩屏 | 全局状态机 + 能量条仪表盘 |
