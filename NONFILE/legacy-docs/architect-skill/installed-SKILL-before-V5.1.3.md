...
description: >
  ...
  1. /simplify (three-way code review) → 2. /init (regenerate AGENTS.md) →
...
**临界区保护 — 首选模式 (V2.1.1)**: 在临界区内将共享缓冲区 `strncpy` 到局部栈数组, 恢复中断后再解析局部副本。彻底消除 ISR 并发修改风险:
...
### 2.5 main.c 极简原则 (V2.0.0: KEY 触发联网 + 静默看门狗)
...
**联网流程**: 上电不自动联网。KEY0 单击触发阻塞联网 (20~30s, 返回 0=成功/1~6=错误码)。成功→巴法云订阅 (cmd=1) + IDLE 待机; 失败→OLED 显示错误码 3 秒→自动回待联网界面→KEY0 可重试。联网成功后再次 KEY0 单击触发软启动扫频。V2.3.0 WAN 分支移除静默看门狗 (巴法云默认静默, 仅靠 CLOSED 帧检测断线)。V2.3.1: WiFi LED 连接成功后常亮 (LED_SOLID), 断线后慢闪。
...
**V2.3.1 浮点精度修复**: `(float)(s_vaccum / s_vfilled)` 改为 `(float)s_vaccum / s_vfilled` — 整数除法在先导致丢失小数 ADC 计数, 电压/电流读数有 ~0.015V 系统性截断误差。
...
├── AGENTS.md      ← 项目指南
├── .Codex/       ← Codex 配置
└── Codex/   ← 所有 AI 生成文件 (docs/, docx/, tools/, superpowers/, package.json)
```

**文件放置铁律**: Keil 编译必需的 `.c/.h/.s/.uvprojx` 保持原位不动。AI 生成的所有文档 (.md/.docx)、工具脚本 (.ps1/.js/.py)、设计文件 (superpowers/) 统一放 `Codex/` 下。
...
| 版本 | 日期 | 变更说明 |
|:---|:---|:---|
| V2.0.0 | 2026-06-17 | 初始版本 — 完整架构白皮书 |
```

**自动迭代规则** (每次输出文档时必须遵守):

1. **逻辑变更 → 升版本号**:
   - 修改了任何 `.c`/`.h` 代码逻辑 → 小版本号 +0.0.1 (如 V2.0.0 → V2.0.1)
   - 新增外设驱动模块 → 中版本号 +0.1.0 (如 V2.0.3 → V2.1.0)
...
| 版本 | 日期 | 变更说明 |
|:---|:---|:---|
| V2.0.2 | 2026-06-20 | App_Net 新增心跳检测与 TCP 断连自动重连 |
| V2.0.1 | 2026-06-17 | 修复 USART2 ORE 溢出中断锁死; PWM 防偏磁强制偶数周期 |
| V2.0.0 | 2026-06-14 | 初始版本 — 完整架构白皮书 |
```
...
- 使用 `docx` npm 包通过 `Codex/tools/generate_docx.js` 批量生成, 从 `Codex/` 目录执行 `npm install && node Codex/tools/generate_docx.js`
...
- **V2.3.1**: `OLED_ShowFloatNum` 参数类型 `double`→`float` (Cortex-M3 无硬件 FPU, double 软件模拟极慢); 新增 `pow10_lut[10]` 查找表替代 `OLED_Pow(10,...)` 运行时循环计算
...
Step 4: 变更分类 & 版本决策
        ├─ 无实质性代码变更 → 不升版本, 仅刷新日期
        ├─ 代码逻辑修改 → +0.0.1 (V2.0.0 → V2.0.1)
        └─ 新增外设模块 → +0.1.0 (V2.0.0 → V2.1.0)
...
Step 8: 重新生成双份文件
        ├─ 覆盖 Codex/docs/<文档名>.md
        └─ 运行 node Codex/tools/generate_docx.js 覆盖对应 .docx
```
...
- AI 生成的文件散落在项目根目录——必须统一放在 `Codex/` 下
