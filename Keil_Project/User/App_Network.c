...
/* ── 指数退避: 15s → 30s → 60s → 120s → 300s → 30min, 不永久 FAILED ── */
static uint32_t App_Network_Get_Retry_Timeout(void)
{
    if (s_retry_count < 6)  return 15000;    /* 0-5:   15s */
    if (s_retry_count < 12) return 30000;    /* 6-11:  30s */
    if (s_retry_count < 20) return 60000;    /* 12-19: 60s */
    if (s_retry_count < 30) return 120000;   /* 20-29: 2min */
    if (s_retry_count < 45) return 300000;   /* 30-44: 5min */
    return 1800000;                           /* 45+:   30min */
}
