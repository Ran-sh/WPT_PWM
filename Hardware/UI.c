 ...
 * @note    纯本地控制版: 无 WiFi/网络依赖, KEY0 单击直接触发软启动
 ******************************************************************************
 */
...
static void UI_UpdateLEDs(SoftStart_State_t ss)
{
    ...
    /* PB5 Ready: SS_FAULT 时灭, 否则 Page0 + 待机/运行 时亮 */
    if (ss == SS_FAULT)
        LED_Update_Ready(0);
    else
...
{
    (void)on_off;  /* 由 Inverter_SoftStart_GetState 推导 */
}
...
{
    (void)on_off;  /* 纯本地版: 无 WiFi, 接口保留兼容 */
}
