...
> 触发词 `更新全部内容` 必须严格按以下 12 步执行，每步标注了 **【针对文件】**，
> 路径规则: 写目录路径表示该目录下全部文件 (含子目录)，禁止跳过、禁止处理不在列表中的文件。

### 第 0 条 — 全局注释审查 (每次必做)

**ARMCC V5 C89 注释铁律:**
- 禁止 Unicode box-drawing (`┌└├│`) -> 用纯 ASCII `+` `-` `|` 替代
- 禁止 `/* */` 嵌套 -> 注释中 C 代码用 `//` 替代
- 中文 UTF-8 允许 -> 但不要混入特殊 Unicode 符号

**注释质量铁律:**
- `.c` 文件头部必须有接线图 (`+--+` ASCII 框)
- `.h` 文件头部简洁, 接线图引用 `.c` 文件
- 每个 `.c` 文件内部: 所有模块状态变量有注释, 关键逻辑行有注释, 公开函数有 `@brief`/`@param`
- 每行代码尽量有行尾注释说明其作用
- `@brief` 统一中英双语格式: `模块名 — V4.3.x`
- 版本号全项目统一 (当前 V4.3.2)

| 针对文件 | 检查内容 |
|:---|:---|
| `Keil_Project/` 下全部 .c | 头部接线图 ASCII, @brief 中文, 内部变量/函数注释完整 |
| `Keil_Project/` 下全部 .h | 头部简洁, 无接线图(引用 .c), 公开接口有 @brief/@param |
| `Arduino_Project/` 下全部 .ino | 头部版本+功能注释 |
| `ch341/` 下全部 .py | 头部版本+变更历史 |
...
| 针对文件 | 写入内容 |
|:---|:---|
| 全部 .c/.h `@brief` 行 | 版本号 V4.3.2 |
| `CLAUDE.md` | `版本: V4.3.2` |
| `README.md` | badge + 版本历史 |
| `ch341/README.md` | 适用项目日期 |
| `ch341/*.py` | 头部 V1.x 变更历史 |
| `Arduino_Project/*.ino` | 头部 @note 版本 |
| `.docx` | 重新生成 |
