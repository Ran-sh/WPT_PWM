...
> 适用项目: WPT_PWM V5.1.0 (`5.0`分支) | 目标芯片: W25Q128 (16MB SPI NOR Flash) | 更新: 2026-07-26

> **V5.1.0 同步说明**: 本次仅同步固件版本与项目文档；CH341A 接线、字库镜像格式和生成、烧录、校验流程均未改变。
...
V5.1.0运行时由 `Spi1_Shared` 管理TFT和W25Q128的共享总线，但它不能解决外部CH341A与已上电STM32同时驱动的问题，因此烧录时仍必须让目标板完全断电。
...
| 烧录成功但屏幕无变化 | STM32 固件未启用 Flash 字库 | 确认使用V5.1.0固件，并检查启动时Flash字库CRC状态 |
| 屏幕白屏 (无任何显示) | PB12虚焊或共享SPI接线冲突 | 检查PB12/PA5/PA6/PA7，确认CH341A排针已拔下；V5.1.0会在超时后恢复总线 |
